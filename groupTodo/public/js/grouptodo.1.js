...
(function(global,$){
  ...
  function init() {
    ...
    // 메모 데이타 주소
    todo_api_address_1 = '/todo1';

    // html 요소 가져오기
    app = document.querySelector('.app');
    todo = document.querySelector('.todo'); // box
    todo_buttons = document.querySelectorAll('button')
    todo_title = document.querySelector('#todo-title1');
    todo_content = document.querySelector('#todo-content1');
  ...
  function load(params) {
    ...
    $.get(todo_api_address_1, function (data, status) {
  ...
  function saveTodo() {
    ...
    //     todo_api_address = "/" + e;   // << 데이터 저장되는 곳
        value = select.value;
    $.post(todo_api_address_1, $.param(todo_item), function (data, status) {
  ...
  function removeTodo(ev) {
    ...
    if (target.localName === 'button' && target.classList.contains('delete')) {
      ...
      $.ajax({
        url: todo_api_address_1 + '/' + remove_id,
        method: 'DELETE',
        dataType: 'json',
